/* BIOL 40B Exam 1: Objective Scope — MCQ engine.
   Pick an answer → correct choice + explanation revealed → Next.
   Missed questions re-queue each round until every one is cleared.
   (Same engine as the Lecture Exam trainer, over the OBJECTIVE bank.) */
(() => {
  const $ = id => document.getElementById(id);
  const LETTERS = ["A","B","C","D","E"];

  /* ---------- theme (shared key with the other trainer pages) ---------- */
